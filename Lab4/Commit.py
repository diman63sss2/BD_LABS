...
class Worker(threading.Thread):
    def __init__(self, worker_id, root):
        super(Worker, self).__init__()

        self.worker_id = worker_id
        self.root = root
        self.path = f"{root}/{worker_id}"
    ...
    def run(self):
        ...
        print(f"Node {self.worker_id} votes {value.decode()}")
        zk.create(self.path, value, ephemeral=True)

        @zk.DataWatch(self.path)
        def watch(data, stat):
            if stat.version > 0:
                print(f"Node {self.worker_id} performs {data.decode()}")
        ...
        def workers_watch(workers):
            if not is_solved[0]:
                if len(workers) == self.n_workers or (datetime.now() - then).total_seconds() > 30:
                    ...
                    for w in workers:
                        vote = zk.get(f"{self.node_home}/{w}")[0]
                        commits += int(vote == commit)
                        aborts += int(vote == abort)
                    ...
                    print(f"Registering nodes: {workers}")
...
if __name__ == "__main__":
    ...
    for worker_id in range(n_workers):
        worker = Worker(worker_id, master.node_home)
